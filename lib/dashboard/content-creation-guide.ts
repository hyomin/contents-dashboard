...
export const GUIDE_BY_CATEGORY: Record<GuideCategory, CategoryGuide> = {
  writing: {
    ...
    checklist: [
      ...
      '«한 줄 요약» 박스 + 마지막 단락: 다음 행동(댓글·구독·저장·공유) 한 줄 CTA',
      'H2 4개 이상이면 본문 최상단에 «📌 목차» 직접 작성',
    ...
    sections: [
      ...
      },
      {
        heading: '인트로 패턴 (택1)',
        bullets: [
          '질문·공감형(기본값): 독자의 궁금증을 1문장 질문으로 제시 → «오늘은 ~을 정리합니다»로 구조 예고',
          '고정 자기소개형: 매 글 동일한 한 문장 자기소개 — 시리즈·정기 발행에 적합',
          '근황·일기형: «오랜만에 글을 올리네요» 식 근황 토크 — 비정기 발행에 적합',
        ],
      },
      {
        heading: '글 유형별 구성 템플릿 (택1)',
        bullets: [
          'A. 시황/매크로형: 한 줄 요약(굵게) → 짧은 단락 전개 → «-소제목-»으로 전환 → 표·이미지 1~2개 → «수혜주는?/참고 글» 마무리',
          'B. 종목/주제분석형: 📌 목차 → 번호형 H2(개요→근거→비교표→포인트/리스크) → 한 줄 요약 박스 → FAQ 2~5개 → 관련 글 추천',
          'C. 재테크/정보형: 이모지 질문 인트로 → ✅/번호 이모지 H2 + 비교표 → «체크포인트» 반복 박스(선택) → FAQ + 필요 시 면책 고지',
        ],
      },
      {
      ...
      {
        heading: '본문 구조 템플릿 (기본형 — 위 유형 템플릿이 안 맞을 때)',
        bullets: [
