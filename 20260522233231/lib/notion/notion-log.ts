...
async function fetchTodayContent(platform: string, dateStr: string) {
  ...
  const { data } = await supabaseAdmin
    .from('videos')
    .select('video_id, title, platform, channel_id, channel_name, views, vs_avg, published_at, updated_at')
...
async function fetchTodayOutliers(dateStr: string) {
  ...
  const { data } = await supabaseAdmin
    .from('outlier_tags')
    .select('video_id, title, platform, tier, vs_avg, channel_name, tagged_at')
...
async function fetchTodayRssTopics(dateStr: string) {
  ...
  const { data } = await supabaseAdmin
    ...
    .select('title, url, source_name, relevance_score, published_at, created_at')
...
function buildRssBlocks(
  ...
  return items.map((item) => {
    const suffix = [
      ...
      .filter(Boolean)
      .join(' · ')
    return bulletBlock(item.title, item.url ?? undefined, suffix || undefined)
  })
}
