        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <TitleWithHint
              ...
              hintVariant="light"
              hint="채널 평균 조회수 대비 성과(vs.Avg)가 기준 배수 이상인 콘텐츠를 필터링합니다. 포맷 탭으로 숏폼·롱폼을 분리해서 볼 수 있습니다."
            >
              🚀 Outlier 분석
            </TitleWithHint>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-white/20 rounded-xl px-2 py-1.5">
              <span className="text-xs mr-1">기준:</span>
              {[1.5, 2.0, 3.0, 5.0].map(v => (
                <button key={v} type="button" onClick={() => setMinVsAvg(v)}
                  className={`text-xs px-2 py-0.5 rounded-lg font-bold transition ${minVsAvg === v ? 'bg-white text-green-700' : 'text-white hover:bg-white/20'}`}>
                  {v}x
                </button>
              ))}
            </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center text-gray-400">
          ...
          <p className="font-medium">
            {formatTab !== 'all'
              ? `${FORMAT_LABELS[formatTab]} Outlier가 없습니다. 포맷 분류가 적용되지 않은 영상은 "전체"에서 확인하세요.`
