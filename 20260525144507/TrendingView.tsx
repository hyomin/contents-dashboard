        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 space-y-3 border border-gray-100 dark:border-gray-700">
          {loading ? (
            ...
            keywords.map((kw) => (
              <div
                key={kw.keyword}
                className="flex items-center gap-4 p-4 rounded-xl hover:bg-orange-50 dark:hover:bg-gray-700 transition group"
              >
                <span
                ...
                {queueItems.some((q) => q.keyword === kw.keyword) ? (
                  <span className="px-3 py-1.5 bg-orange-100 text-orange-600 text-xs rounded-lg font-medium">
                    ✓ 큐 추가됨
                  </span>
                ) : (
                  <button
                    ...
                    onClick={() => {
                      ...
                      addToast(
                        added ? `"${kw.keyword}" 기획 큐에 추가됨 ✓` : `"${kw.keyword}"은 이미 큐에 있습니다`,
