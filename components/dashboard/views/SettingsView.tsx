...
const THEME_OPTIONS: ThemeOption[] = [
  ...
  { value: 'soft', label: 'Soft', icon: '🌤️', desc: '밤에 보기 좋은 밝은 회색 톤' },
      ...
      <section className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <div className="mb-5">
          <TitleWithHint
            ...
            hint="Light·Soft·Dark·System 중 선택합니다. Soft는 다크보다 밝은 회색 톤으로 야간에 눈의 피로를 줄입니다."
          >
            🎨 테마
          </TitleWithHint>
          ...
          {THEME_OPTIONS.map((opt) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center gap-3 p-4 sm:p-5 rounded-xl border-2 transition
                  ${
                    isActive
              ...
              >
                <div
                  className={`w-full h-14 rounded-lg border overflow-hidden flex ${previewClass(opt.value)}`}
                >
                  <div
                ...
                {isActive && (
                  <span className="absolute top-2 right-2 w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                    ✓
                  </span>
                )}
              </button>
